...
def superviser(state: NovelGenerationState, user_input):
    # 判断用户输入是否包含"修改小说"或"重开小说"
    if "修改" in user_input or "modify" in user_input.lower():
        return {"next_step": "update_novel"}
    elif "重开" in user_input or "remake" in user_input.lower():
        return {"next_step": "generate_novel"}
    else:
        return {"next_step": END}  # 结束状
def router(state):
...
# 定义工作流
def langgraph_LLM_gpt(model, apikey, url, user_input):
    # 获取模型
    ...
    # 添加监督者节点
    workflow.add_node("superviser", lambda state: superviser(state, user_input))

    # 显式添加边
    workflow.add_edge(START, "generate_novel")  # 从 START 到 generate_novel
    workflow.add_edge("generate_novel", "superviser")   # 从 generate_novel 到 superviser
    ...
    # 初始化状态
    initial_state = {"user_input": user_input, "novel": None, "next_step": "generate_novel"}
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if user_input.lower() in ["退出", "exit"]:
            print("对话结束。再见！")
            break
        if user_input.lower() in ["修改", "update"]:
            print("请输入修改的内容：")
            user_input = input("修改内容: ")
